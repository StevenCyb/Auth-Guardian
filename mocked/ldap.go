...
// LDAPListenerCloseFlag flag to close listener
var LDAPListenerCloseFlag bool

func init() {
	LDAPListenerCloseFlag = false
}

// RunMockLDAPIDP runs a mocked LDAP IDP
func RunMockLDAPIDP() *net.Listener {
	// Overrite config
	...
	go func() {
		for {
			...
			if err != nil || LDAPListenerCloseFlag {
				return
			}
